...
def load_seen() -> dict:
    """
    Load previously seen jobs from disk. Returns empty dict on first run.

    Format: { "CompanyName::JobID": "Job Title", ... }

    Backwards compatible with the old list format (list of UID strings).
    If the existing seen_jobs.json is a list, it is automatically migrated
    to the new dict format — titles will be empty strings for migrated entries.
    """
    if os.path.exists(SEEN_FILE):
        with open(SEEN_FILE) as f:
            data = json.load(f)
        if isinstance(data, list):
            # Migrate old format (list of UIDs) to new dict format
...
def save_seen(seen: dict):
    """Persist the updated seen-jobs dict to disk after each run."""
...
def run():
    ...
    for company in companies:
        ...
        for raw in raw_jobs:
            ...
            # Store the title alongside the UID for human readability.
            seen[uid] = job.get("title", "")
