...
def matches_title_d1(title: str, d1: dict) -> bool:
    ...
    if not any(s in t for s in d1["seniority"]):
        return False
...
def is_relevant(job: dict, d1: dict, d3_signals: list) -> bool:
    ...
    if matches_department_d3(dept, d3_signals) and any(s in t for s in d1["seniority"]):
        return True
...
def score_job(job: dict, cluster_hits: int, scoring: dict) -> dict:
    ...
    # "director" is listed last because it's a substring of "senior director"
    # and "associate director" — order prevents under-counting.
    sen_scores = {k: v for k, v in scoring["seniority_scores"].items()
                  if not k.startswith("_")}
    sen = 0
    for lvl, pts in sen_scores.items():
        if lvl in title:
            sen = pts
            break
    score += sen
...
def fetch_workday(company: dict, clusters: list) -> tuple:
    ...
    def _query(search_text: str, limit: int = 20) -> list:
        ...
        while True:
            ...
            # Some Workday tenants (e.g. vhr-hosted instances) reject appliedFacets
            # in the POST body and return 422. Only include it for standard tenants.
            if not company.get("no_search_text"):
                body["appliedFacets"] = {}
